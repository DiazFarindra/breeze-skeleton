                    ...
                    <div class="min-w-full align-middle">
                        ...
                        <form action="{{ route('users.update', $account->id) }}" method="post">
                            ...
                            <div class="mb-3">
                                ...
                                <x-input id="name" class="block w-full mt-1" type="text" name="name" :value="$account->name ?? old('name')" autofocus />
                            </div>
                            <div class="mb-3">
                                <x-label for="email" :value="__('email')" />
                                <x-input id="email" class="block w-full mt-1" type="email" name="email" :value="$account->email ?? old('email')" autofocus />
                            </div>
                            <div class="mb-3">
                                ...
                                <select name="roles[]" id="select2" class="w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" multiple>
                                    @forelse ($roles as $role)
                                        <option @selected($account->roles->pluck('id')->contains($role->id)) value="{{ $role->id }}">{{ $role->name }}</option>
                                    @empty
